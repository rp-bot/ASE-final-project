...
namespace Audio
{
    ...
    void VoiceManager::ensureVoicesAndSoundAdded()
    {
        ...
        for (int i = 0; i < MAX_VOICES; ++i)
            m_synthesiser.addVoice (new SynthVoice (m_wavetableBank.get()));
    ...
    void VoiceManager::setBlockParameterSnapshot (const ParameterSnapshot& snapshot) noexcept
    {
        m_blockSnapshot = snapshot;
        for (int i = 0; i < MAX_VOICES; ++i)
        {
            if (auto* v = m_synthesiser.getVoice (i))
                static_cast<SynthVoice*> (v)->setParameterSnapshot (&m_blockSnapshot);
        }
    }
